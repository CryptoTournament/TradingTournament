...
const PositionTable = ({ positions, currentPrice, players }) => {
  ...
  const filteredPositions = closedPositions.filter((position) => {
    ...
    return closePrice !== 0 && userDisplayName.includes(searchValue.toLowerCase());
  });
  ...
  return (
    <div className="flex flex-col items-center w-full mt-8 xl:mt-0 px-4">
      <div className="flex xl:flex-col">
        <h2 className="text-xl font-bold mb-5 mr-8 xl:mr-0 xl:ml-8 text-white">
          Closed Positions
        </h2>
      ...
      <table className="min-w-full bg-opacity-0">
        <thead className="bg-black text-teal-600 uppercase">
            <tr>
              ...
              <th className="py-2 px-2 sm:px-4 w-1/6 text-center">TRADE SIZE</th>
              <th className="py-2 px-2 sm:px-4 w-1/6 text-center">
                ROE
              </th>
